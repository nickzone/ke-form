...
export default class FormStore extends Component {
  ...
  normalizeFormData = () => {
    ...
    formConfig.fields.forEach((field) => {
      const { name } = field;
      const initialValue = this.getFieldDefaultValue(field);
  ...
  getFieldDefaultValue = (field) => {
    return 'initialValue' in field ? field.initialValue : getField(field.type, 'initialValue');
  }
  ...
  changeField = (name, value) => {
    ...
    const defaultValue = this.getFieldDefaultValue(
      getFieldByName(name, this.getFields())
    );
    this.setState((state) => {
      return {
        formData: {
